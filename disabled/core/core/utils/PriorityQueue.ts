...
export class PriorityQueue<T> {
    ...
    public heapSort(): Array<Optional<T>> {
        return Array.from({length: this.length}, () => this.dequeue());
    }

    private parent(nodeIndex: number): Optional<number> {
        if (nodeIndex === 0) {
            return null
        }
    ...
    private leftChild(nodeIndex: number): Optional<number> {
    ...
    private rightChild(nodeIndex: number): Optional<number> {
    ...
    private bubbleUp(nodeIndex: number = this.length - 1): void {
    ...
    public bubbleDown(nodeIndex: number = 0): void {
